...
CREATE TABLE `ali_payconfig`  (
  ...
  `ali_pay_public_key` varchar(1000)  NOT NULL,
  `app_id` varchar(50)  NOT NULL,
  `service_url` varchar(200)  NOT NULL,
  `charset` varchar(20)  NOT NULL,
  `sign_type` varchar(10)  NOT NULL,
  `format` varchar(10)  NOT NULL,
  `cert_path` varchar(200)  NOT NULL,
  `alipay_public_cert_path` varchar(200)  NOT NULL,
  `root_cert_path` varchar(200)  NOT NULL,
  `encrypt_type` varchar(50)  NOT NULL,
  `aes_key` varchar(50)  NOT NULL,
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `create_user_id` varchar(50)  NOT NULL,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_user_id` varchar(50)  NOT NULL,
  `active` int NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
...
CREATE TABLE `t_menu`  (
  ...
  `page_url` varchar(3000)  NOT NULL,
  `menu_type` int NOT NULL COMMENT '0.功能按钮,1.导航菜单',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `create_user_id` varchar(50)  NOT NULL,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_user_id` varchar(50)  NOT NULL,
...
CREATE TABLE `t_org`  (
  ...
  `org_type` int NOT NULL COMMENT '0-99门店,100-199部门,200-299,分公司,300-399集团公司,900-999总平台',
  `sortno` int NOT NULL COMMENT '排序,查询时倒叙排列',
...
CREATE TABLE `t_role`  (
  ...
  `private_org` int NOT NULL DEFAULT 0 COMMENT '角色的部门是否私有,0否,1是,默认0.当角色私有时,菜单只使用此角色的部门权限,不再扩散到全局角色权限,用于设置特殊的菜单权限.公共权限时部门主管有所管理部门的数据全权限,无论角色是否分配. 私有部门权限时,严格按照配置的数据执行,部门主管可能没有部门权限.',
  `role_org_type` int NOT NULL DEFAULT 0 COMMENT '0自己的数据,1所在部门,2所在部门及子部门数据,3.自定义部门数据.',
  `org_id` varchar(50)  NOT NULL COMMENT '角色的归属部门,只有归属部门的主管和上级主管才可以管理角色,其他人员只能增加归属到角色的人员.不能选择部门或则其他操作,只能添加人员,不然存在提权风险,例如 员工角色下有1000人, 如果给 角色 设置了部门,那这1000人都起效了.',
  `share_role` int NOT NULL DEFAULT 0 COMMENT '角色是否共享,0否 1是,默认0,共享的角色可以被下级部门直接使用,但是下级只能添加人员,不能设置其他属性.共享的角色一般只设置roleOrgType,并不设定部门.',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `t_role_org`  (
  `id` varchar(50)  NOT NULL COMMENT '编号',
  `org_id` varchar(50)  NOT NULL COMMENT '部门编号',
  `role_id` varchar(50)  NOT NULL COMMENT '角色编号',
  `children` int NOT NULL DEFAULT 0 COMMENT '0不包含子部门,1包含.用于表示角色和部门的权限关系',
  `bak1` varchar(100)  NULL DEFAULT NULL,
  `bak2` varchar(100)  NULL DEFAULT NULL,
  `bak3` varchar(100)  NULL DEFAULT NULL,
  `bak4` varchar(100)  NULL DEFAULT NULL,
...
CREATE TABLE `t_user`  (
  ...
  `union_id` varchar(200)  NOT NULL COMMENT '微信UnionID',
  `avatar` varchar(2000)  NOT NULL COMMENT '头像地址',
  `user_type` int NOT NULL COMMENT '0会员,1员工,2店长收银,9系统管理员',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `create_user_id` varchar(50)  NOT NULL,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_user_id` varchar(50)  NOT NULL,
...
CREATE TABLE `t_user_org`  (
  ...
  `manager_type` int NOT NULL DEFAULT 1 COMMENT '0会员,1员工,2主管',
  `bak1` varchar(100)  NULL DEFAULT NULL,
  `bak2` varchar(100)  NULL DEFAULT NULL,
  `bak3` varchar(100)  NULL DEFAULT NULL,
  `bak4` varchar(100)  NULL DEFAULT NULL,
...
CREATE TABLE `t_user_platform_infos`  (
  ...
  `user_id` varchar(50)  NOT NULL COMMENT 't_user表中ID',
  `open_id` varchar(100)  NOT NULL COMMENT '公众号openId,企业号userId,小程序openId,APP推送deviceToken',
  `device_type` int NOT NULL COMMENT '设备/应用类型：1公众号2小程序3企业号4APP IOS消息推送5APP安卓消息推送6web',
  `org_id` varchar(50)  NOT NULL COMMENT '所属组织机构ID',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `create_user_id` varchar(50)  NOT NULL,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_user_id` varchar(50)  NOT NULL,
...
CREATE TABLE `wx_cpconfig`  (
  `id` varchar(50)  NOT NULL,
  `org_id` varchar(50)  NOT NULL COMMENT '站点Id',
  `app_id` varchar(500)  NOT NULL COMMENT '开发者Id',
...
CREATE TABLE `wx_miniappconfig`  (
  ...
  `app_id` varchar(500)  NOT NULL COMMENT '开发者Id',
  `secret` varchar(500)  NOT NULL COMMENT '应用密钥',
  `plan_id` varchar(500)  NOT NULL COMMENT '签约模板Id',
  `request_serial` varchar(5000)  NOT NULL COMMENT '签约请求序列号',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `create_user_id` varchar(50)  NOT NULL,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_user_id` varchar(50)  NOT NULL,
...
CREATE TABLE `wx_mpconfig`  (
  ...
  `aes_key` varchar(500)  NOT NULL COMMENT '消息加解密密钥',
  `wx_original_id` varchar(500)  NOT NULL COMMENT '微信原始ID',
  `oauth2` int NOT NULL DEFAULT 1 COMMENT '是否支持微信oauth2.0协议,0是不支持,1是支持',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `create_user_id` varchar(50)  NOT NULL,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_user_id` varchar(50)  NOT NULL,
...
CREATE TABLE `wx_payconfig`  (
  ...
  `app_id` varchar(500) NOT NULL COMMENT '开发者Id',
  `secret` varchar(500) NOT NULL COMMENT '应用密钥',
  `mch_id` varchar(500) NOT NULL COMMENT '微信支付商户号',
  `mch_api_v3_key` varchar(500) NOT NULL COMMENT '商户API v3密钥 (32字节)',
  `wechat_pay_certificate` varchar(500) NOT NULL COMMENT '微信支付平台证书地址',
  `notify_url` varchar(1000) NOT NULL COMMENT '通知地址',
  `cert_serial_no` varchar(1000) NOT NULL COMMENT '商户证书序列号',
  `private_key` varchar(1000) NOT NULL COMMENT '商户API私钥,地址',
  `sign_type` varchar(255) NOT NULL COMMENT '加密方式,MD5和HMAC-SHA256',
...
-- 测试用的存储过程
DELIMITER //
CREATE PROCEDURE testproc(IN demo_id VARCHAR(50))
BEGIN
     SELECT * FROM `t_demo` WHERE id=demo_id;
END ;
//
DELIMITER ;
...
DELIMITER //
CREATE  FUNCTION   testfunc(user_id VARCHAR(50))
RETURNS VARCHAR(30)
BEGIN
declare return_value VARCHAR(30) default '';
SELECT user_name into return_value FROM `t_demo` WHERE id=demo_id;
return return_value;
END
//
DELIMITER;
