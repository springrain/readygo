...
CREATE TABLE `t_org`  (
  ...
  `orgType` int NOT NULL COMMENT '0-99门店,100-199部门,200-299,分公司,300-399集团公司,900-999总平台',
...
CREATE TABLE `t_role`  (
  ...
  `privateOrg` int NOT NULL DEFAULT 0 COMMENT '角色的部门是否私有,0否,1是,默认0.当角色私有时,菜单只使用此角色的部门权限,不再扩散到全局角色权限,用于设置特殊的菜单权限.公共权限时部门主管有所管理部门的数据全权限,无论角色是否分配. 私有部门权限时,严格按照配置的数据执行,部门主管可能没有部门权限.',
  `roleOrgType` int NOT NULL DEFAULT 0 COMMENT '0自己的数据,1所在部门,2所在部门及子部门数据,3.自定义部门数据.',
  ...
  `shareRole` int NOT NULL DEFAULT 0 COMMENT '角色是否共享,0否 1是,默认0,共享的角色可以被下级部门直接使用,但是下级只能添加人员,不能设置其他属性.共享的角色一般只设置roleOrgType,并不设定部门.',
  `createTime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `t_user`  (
  ...
  `userType` int NOT NULL COMMENT '0会员,1员工,2店长收银,9系统管理员',
  `createTime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `createUserId` varchar(50)  NOT NULL,
  `updateTime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updateUserId` varchar(50)  NOT NULL,
...
CREATE TABLE `t_user_org`  (
  ...
  `managerType` int NOT NULL DEFAULT 1 COMMENT '0会员,1员工,2主管',
  `bak1` varchar(100)  NULL DEFAULT NULL,
  `bak2` varchar(100)  NULL DEFAULT NULL,
  `bak3` varchar(100)  NULL DEFAULT NULL,
  `bak4` varchar(100)  NULL DEFAULT NULL,
...
CREATE TABLE `t_user_platform_infos`  (
  ...
  `deviceType` int NOT NULL COMMENT '设备/应用类型：1公众号2小程序3企业号4APP IOS消息推送5APP安卓消息推送6web',
...
CREATE TABLE `wx_payconfig`  (
  ...
  `mchAPIv3Key` varchar(500) NOT NULL COMMENT '商户API v3密钥 (32字节)',
  `wechatPayCertificate` varchar(500) NOT NULL COMMENT '微信支付平台证书地址',
  `notifyURL` varchar(1000) NOT NULL COMMENT '通知地址',
  `certSerialNo` varchar(1000) NOT NULL COMMENT '商户证书序列号',
  `privateKey` varchar(1000) NOT NULL COMMENT '商户API私钥,地址',
