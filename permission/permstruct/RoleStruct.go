...
type RoleStruct struct {
	...
	PrivateOrg int `column:"private_org"`

	// RoleOrgType 0自己的数据,1所在部门,2所在部门及子部门数据,3.自定义部门数据.
	RoleOrgType int `column:"role_org_type"`
	...
	ShareRole int `column:"share_role"`

	// CreateTime <no value>
	CreateTime time.Time `column:"create_time"`

	// CreateUserId <no value>
	CreateUserId sql.NullString `column:"create_user_id"`

	// UpdateTime <no value>
	UpdateTime time.Time `column:"update_time"`

	// UpdateUserId <no value>
	UpdateUserId sql.NullString `column:"update_user_id"`
